...
impl Anonymizer {
    ...
    fn anonymize_bin(&mut self, addr: u128, version: u8) -> Result<u128, ErrorStack> {
        ...
        for pos in 0..pos_max {
            ...
            let mut encrypted = [0; 16 + 16];
            let mut _cnt = 0;
            _cnt += self.crypter.update(&padded_bytes, &mut encrypted)?;
            _cnt += self.crypter.finalize(&mut encrypted[_cnt..])?;
...
mod tests {
    ...
    fn run_test_cases(cases: &[(&str, &str)]) -> Result<(), ErrorStack> {
        let mut pancake = Anonymizer::new(KEY)?;
